...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/BALLCOINParams"
    else
        PARAMS_DIR="$HOME/.ballcoin-params"
    fi
fi
...
Failed to fetch the BALLCOIN zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
BALLCOIN - fetch-params.sh

This script will fetch the BALLCOIN zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common BALLCOIN zkSNARK parameters. Note that it is
