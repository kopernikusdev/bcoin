...
def remove_current_translations():
    ...
    This leaves ballcoin_en.ts untouched.
    '''
...
def update_build_systems():
    ...
    '''
    filename_lang = [re.match(r'((ballcoin_(.*)).ts)$', filename).groups() for (filename, filepath) in all_ts_files(include_source=True)]
    filename_lang.sort(key=lambda x: x[0])

    # update qrc locales
    with open('src/qt/ballcoin_locale.qrc', 'w') as f:
