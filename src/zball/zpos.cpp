...
bool CLegacyZBallStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZBallStake from non zc-spend", __func__);
    ...
    *this = CLegacyZBallStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zball stake origin", __func__);

    // All good
    return true;
}

CLegacyZBallStake::CLegacyZBallStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZBallStake::GetIndexFrom()
{
...
CAmount CLegacyZBallStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZBallStake::GetUniqueness() const
{
...
bool CLegacyZBallStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zBALL stake block: height %d outside range", __func__, nHeight);
