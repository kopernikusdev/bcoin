...
UniValue getinfo(const JSONRPCRequest& request)
{
    ...
    UniValue supply_info = getsupplyinfo(JSONRPCRequest());
        obj.push_back(Pair("moneysupply", supply_info["supply"]));
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for BALLCOIN cold staking -only if is standard address-\n"
