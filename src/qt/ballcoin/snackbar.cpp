...
#include "qt/ballcoin/forms/ui_snackbar.h"
#include "qt/ballcoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(BALLCOINGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &BALLCOINGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
