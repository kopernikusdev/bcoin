...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    ...
    if (settings.contains("nAnonymizeBALLcoinAmount") || reset)
        SoftSetArg("-anonymizeballcoinamount", settings.value("nAnonymizeBALLcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ballcoin.cpp during shotdown to also remove the window positions
