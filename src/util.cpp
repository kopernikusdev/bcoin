...
const char * const BALLCOIN_CONF_FILENAME = "ballcoin.conf";
const char * const BALLCOIN_PID_FILENAME = "ballcoin.pid";
const char * const BALLCOIN_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BALLCOINParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BALLCOINParams
    // Mac: ~/Library/Application Support/BALLCOINParams
    // Unix: ~/.ballcoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BALLCOINParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "BALLCOINParams";
#else
    // Unix
    return pathRet / ".ballcoin-params";
#endif
#endif
}
...
{
    fs::path pathConfigFile(GetArg("-conf", BALLCOIN_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(GetArg("-mnconf", BALLCOIN_MASTERNODE_CONF_FILENAME));
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", BALLCOIN_PID_FILENAME));
