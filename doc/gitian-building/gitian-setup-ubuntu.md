...
The build script we'll be using is contained in the BALLCOIN github repository ([contrib/gitian-build.py](https://github.com/kopernikusdev/bcoin/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the BALLCOIN repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/kopernikusdev/bcoin/master/contrib/gitian-build.py
