...
*Setup instructions for a gitian build of BALLCOIN Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the BALLCOIN
...
to the BALLCOIN GitHub release page.
...
- [Building BALLCOIN Core](#building-ballcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/ballcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/ballcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/ballcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/ballcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/ballcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/ballcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/ballcoin-project/gitian.sigs/) repository:
...
```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/ballcoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/ballcoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/ballcoin-osx-*-build.assert
```
